...
function BlogListByLabel() {
  ...
  // ===== Fetch blogs by label =====
  useEffect(() => {
    if (!label) return;
    fetch(
      `http://localhost:8083/api/blogs/public/by-label?label=${encodeURIComponent(
        label
      )}`
    )
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch blogs by label");
        return res.json();
      })
      .then((data) => {
        console.log("✅ Blogs by label:", data);
        setBlogs(data || []);
      })
  ...
  if (!label) return <p>No label specified.</p>;
    ...
    <div className="wrapper">
      <section className="content">
        <div className="container-fluid">
          <h3>
            Posts with label: <span className="text-primary">{label}</span>
          </h3>
          <div className="row">
            {/* Blog List */}
            <div className="col-md-8">
              {blogs.length === 0 ? (
                <p>No blog posts found for this label.</p>
              ) : (
                blogs.map((blog) => (
...
export default BlogListByLabel;
